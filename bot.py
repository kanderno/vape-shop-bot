...
CATEGORY, FLAVOR, QUANTITY, PHONE, ADDRESS, CONFIRM = range(6)
...
def build_category_keyboard() -> InlineKeyboardMarkup:
    ...
    for cat_idx, category in enumerate(storage.get_categories()):
        buttons.append(
            [InlineKeyboardButton(category["title"], callback_data=f"cat:{cat_idx}")]
        )
    buttons.append([InlineKeyboardButton("❌ Отменить заказ", callback_data="cancel_order")])
    return InlineKeyboardMarkup(buttons)


def build_flavor_keyboard(cat_idx: int) -> InlineKeyboardMarkup:
    buttons = []
    category = storage.get_categories()[cat_idx]
    for flavor_idx, flavor in enumerate(category["flavors"]):
        if not flavor["available"]:
            continue
        buttons.append(
            [InlineKeyboardButton(flavor["name"], callback_data=f"flavor:{cat_idx}:{flavor_idx}")]
        )
    buttons.append([InlineKeyboardButton("⬅️ Назад к выбору одноразки", callback_data="back_categories")])
    buttons.append([InlineKeyboardButton("❌ Отменить заказ", callback_data="cancel_order")])
    return InlineKeyboardMarkup(buttons)
...
async def order_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🛍 Какую одноразку хотите заказать?",
        reply_markup=build_category_keyboard(),
    )
    return CATEGORY


async def order_category_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()

    if query.data == "cancel_order":
        return await order_cancel_inline(update, context)
    ...
    await query.edit_message_text(
        f"{category['title']}\n\n🍓 Выберите вкус:",
        reply_markup=build_flavor_keyboard(cat_idx),
    )
    return FLAVOR
...
async def order_flavor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _, cat_idx, flavor_idx = query.data.split(":")
    cat_idx, flavor_idx = int(cat_idx), int(flavor_idx)
    flavor = storage.get_categories()[cat_idx]["flavors"][flavor_idx]

    if not flavor["available"]:
        # На случай если вкус закончился ровно в момент выбора
        await query.edit_message_text(
            f"😔 «{flavor['name']}» только что закончился.\n\n🍓 Выберите другой вкус:",
            reply_markup=build_flavor_keyboard(cat_idx),
        )
        return FLAVOR
    ...
    # Убираем инлайн-кнопки у предыдущего сообщения, чтобы не жали повторно
    await query.edit_message_text(f"✅ Выбрано: {flavor['name']}")

    await query.message.reply_text(
        "🔢 Сколько штук хотите заказать?", reply_markup=cancel_kb()
    )
    return QUANTITY


async def order_cancel_inline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отмена заказа, когда пользователь нажал инлайн-кнопку (выбор категории/вкуса)."""
    query = update.callback_query
    context.user_data.clear()
...
def main():
    ...
    order_conv = ConversationHandler(
        ...
        states={
            CATEGORY: [CallbackQueryHandler(order_category_selected, pattern="^(cat:|cancel_order)")],
            FLAVOR: [CallbackQueryHandler(order_flavor, pattern="^(flavor:|back_categories|cancel_order)")],
